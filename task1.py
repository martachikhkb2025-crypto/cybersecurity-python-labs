...
# (додано порожній рядок та рядок з пробілами для перевірки нової умови)
...
def check_password_strength(password, criteria, forbidden):
    # ПЕРЕВІРКА: Якщо пароль порожній або складається лише з пробілів — не пускаємо далі
...
# Трохи збільшив ширину колонок, щоб вмістити текст "Порожній пароль!"
print(f"{'Пароль':<20}|{'Надійність':<20}")
print("-" * 41)
...
for password in passwords:
    ...
    # Якщо пароль порожній, виводимо <порожньо>, щоб таблиця виглядала рівно
    display_password = "<порожньо>" if not password or not password.strip() else password
    
    # Друкуємо результат у вигляді красивого рядка таблиці
    print(f"{display_password:<20}|{strength:<20}")